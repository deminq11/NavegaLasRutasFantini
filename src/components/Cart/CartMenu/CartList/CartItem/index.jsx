...
export default function CartItem ({item}) {
    ...
    return(
      <div className="group flex flex-row h-full bg-white border border-gray-200 shadow-sm rounded-xl dark:bg-neutral-900 dark:border-neutral-700 dark:shadow-neutral-700/70">
        <div style={{ backgroundImage: `url(${item.images[0]})` }} className="w-1/4  h-30 overflow-hidden bg-cover rounded-l-xl">
        </div>
        <div className="flex-row p-4 md:p-10">
            <Link to={`/product/${item.id}`} className="w-fit transition block mb-1 text-xl dark:text-neutral-300 dark:hover:text-violet-200">
              {item.name}
            </Link>
            <Link to={`/product/${item.id}`} className="transition text-gray-500 hover:text-gray-100 dark:text-neutral-500 dark:hover:text-neutral-300 line-clamp-1 text-ellipsis">
              {item.description}
            </Link>
        </div>
